...
def main():
    ...
    parser.add_argument(
        '-m', '--model', default='dall-e-3',
        help='Model to use, default is "dall-e-3".'
    )
    parser.add_argument(
        '-p', '--prompt', required=True,
        help='Prompt for the generation (obligatory).'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '-s', '--size', type=validate_size, default='1024x1024',
        help='Size of the generated image, default is "1024x1024".'
    )
    parser.add_argument(
        '--quality', type=validate_quality, default='standard',
        help="Image quality: 'standard' or 'hd'. Default is 'standard'."
    )
    parser.add_argument(
        '--style', type=validate_style,
        ...
        if args.output == 'json':
            ...
            for item in result['data']:
                ...
                if args.download:
                    ...
                    download_image(
                        item['url'], filename, args.filename
                    )
